...
function processFile(filePath) {
  ...
  // Match all import/export from relative paths without extension
  const regex = /((?:import\s+(?:type\s+)?(?:(?:\{[^}]*\}|[a-zA-Z_$][a-zA-Z0-9_$]*)(?:\s*,\s*(?:type\s+)?(?:\{[^}]*\}|[a-zA-Z_$][a-zA-Z0-9_$]*))*(?:\s*,\s*)?|default\s+(?:type\s+)?[a-zA-Z_$][a-zA-Z0-9_$]*|type\s+[a-zA-Z_$][a-zA-Z0-9_$]*\s*,\s*)?|export\s+(?:type\s+)?(?:\*|{}\s+))\s+from\s+)'((\.[^']+))'/g;
