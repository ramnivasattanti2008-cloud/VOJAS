 ...
 */
export default function LiveTicker({ items = SAMPLE }: { items?: TickerItem[] }) {
  const [index, setIndex] = useState(0);

  useEffect(() => {
    const t = setInterval(() => setIndex((i) => (i + 1) % items.length), 2400);
    ...
    <div className="relative overflow-hidden glass rounded-xl border border-white/5 py-2.5">
      ...
      <div className="flex items-center gap-6 px-4">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <span className={`text-[10px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded ${CATEGORIES[items[index].category].bg} ${CATEGORIES[items[index].category].color}`}>
              {items[index].category}
            </span>
            <span className="text-xs text-slate-400">{items[index].label}</span>
            <span className="text-sm font-bold text-white tabular-nums">{items[index].value}</span>
            <span className={`flex items-center gap-0.5 text-[10px] font-bold ${
              items[index].delta > 0 ? "text-green-400" :
              items[index].delta < 0 ? "text-red-400" :
              "text-slate-400"
            }`}>
              {items[index].delta > 0 ? <TrendingUp className="w-2.5 h-2.5" /> :
               items[index].delta < 0 ? <TrendingDown className="w-2.5 h-2.5" /> :
               <Minus className="w-2.5 h-2.5" />}
              {Math.abs(items[index].delta).toFixed(1)}%
            </span>
          </motion.div>
        </AnimatePresence>
              ...
              return (
                <div key={`${it.id}-${i}`} className="flex items-center gap-2 shrink-0">
                  ...
                  <span className={`text-[9px] font-bold ${
                    it.delta > 0 ? "text-green-400/70" :
                    it.delta < 0 ? "text-red-400/70" :
                    "text-slate-500"
                  }`}>
