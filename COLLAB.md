...
- **Last 2 Commits by Claude** (neither pushed yet, per instruction):
  - `ed7ae8a` — "feat(investigations): add evidence, investigation and referral services with routes and tests"
  - `ea45851` — "fix(satellite): live CDSE catalog, cross-project observation scoping bug, honest INSUFFICIENT_DATA states, unified timeline, redis retry noise" — see the commit body for the full list; short version: replaced a dead CDSE STAC endpoint with the live public one (no credential needed for catalog search), fixed a Prisma uniqueness bug where two projects sharing a Sentinel-2 tile couldn't both get real observations, fixed two places that silently reported "no change"/fabricated a 50% score instead of an honest INSUFFICIENT_DATA when no pixel data existed, wired satellite observations + citizen reports into the unified project timeline (nothing wrote to it before), and cut a ~6.5-minute Redis retry-noise burst down to ~6s at boot. 39 new unit tests, all passing. Verified end-to-end against the live database and the real CDSE catalogue for all 5 seeded projects.
- **Verified before both handoffs**: `pnpm -r --no-bail typecheck` clean (6/6 workspaces); `pnpm test` green; no fabricated values found, honest-state vocabulary used consistently; one `as any` removed from `investigations.ts` (real type guard against `REFERRAL_AUTHORITIES` instead).
- **Current Sprint / Task**: Phase 2 Investigations/Referrals/Evidence — DONE. Satellite/GIS backend audit — DONE (backend half only). Both ready for your build verification / push.
