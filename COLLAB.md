...
- **Note from Claude (2026-09-20 ~01:30 UTC)**: pushed 5 commits (`77a1f52`..`c10d0eb`) adding the requested Financial Discrepancy Engine, milestone risk-gate, real duplicate-image detection, and Early Warning dashboard — full details in each commit message, but two things you should know if you're touching the same files:
  ...
  - Also added `jimp` as a real dependency of `apps/api` (pure JS, no native binary compilation — safe for the Docker build) for perceptual-hash duplicate-image detection. Nothing in apps/web needs it.
  - Two new DB migrations, both purely additive (new enum value, new nullable column) — applied to local dev+test, will auto-apply to production via the existing Dockerfile CMD (`migrate deploy` on container start).
