 ...
 *
 * The original M5 in-process queue. Kept as a fallback when Redis is
 * unavailable (so local dev works without spinning up Redis). The public
 * satelliteJobQueue.ts dispatches to this OR the BullMQ version depending
 * on whether Redis is reachable.
 *
...
class InProcessSatelliteJobQueue {
  ...
  private async process(jobId: string, retryDelayMs = 0): Promise<void> {
    ...
    if (retryDelayMs > 0) {
      ...
      logger.info(`[sat-queue:inproc] Job ${jobId} scheduling retry #${job.retryCount + 1} in ${retryDelayMs}ms`);
    ...
    try {
      ...
      logger.info(`[sat-queue:inproc] Job ${jobId} completed: ${result.status}`, {
        jobId,
      ...
      job.error = errorMsg;
      logger.error(`[sat-queue:inproc] Job ${jobId} failed (attempt ${job.retryCount + 1}/${MAX_RETRIES}):`, {
        jobId,
        projectId: job.projectId,
...
export const inProcessQueue = new InProcessSatelliteJobQueue();
