...
export function SatellitesClient() {
  const { t } = useLanguage();
    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div>
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-200 text-xs font-semibold uppercase tracking-wider mb-2">
          ...
          <span>{t('satelliteView.badge', 'ISRO NavIC & Earth Observation Telemetry')}</span>
        </div>
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">
          {t('satelliteView.title', 'Indigenous Satellite Geotagging & Physical Change Engine')}
        </h1>
        <p className="text-sm text-slate-600 mt-2 max-w-3xl leading-relaxed">
          {t(
            'satelliteView.desc',
            "VOJAS combines India's indigenous ISRO NavIC sovereign positioning system with ISRO Bhuvan Geoportal and Copernicus Sentinel-2 L2A optical bands for tamper-resistant infrastructure verification."
          )}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-orange-200/80 bg-linear-to-b from-white to-orange-50/20">
          <CardBody className="space-y-3">
            ...
            <h3 className="font-bold text-slate-900 text-sm">
              {t('satelliteView.navicGeotagging', 'ISRO NavIC Geotagging')}
            </h3>
        ...
        <Card className="border-purple-200/80">
          <CardBody className="space-y-3">
            ...
            <h3 className="font-bold text-slate-900 text-sm">
              {t('satelliteView.multispectral', 'Multispectral Band Analysis')}
            </h3>
            <p className="text-xs text-slate-600 leading-relaxed">
        ...
        <Card className="border-blue-200/80">
          <CardBody className="space-y-3">
            ...
            <h3 className="font-bold text-slate-900 text-sm">
              {t('satelliteView.temporalTimeline', 'Temporal Timeline Passes')}
            </h3>
            <p className="text-xs text-slate-600 leading-relaxed">
        ...
        <Card className="border-emerald-200/80">
          <CardBody className="space-y-3">
            ...
            <h3 className="font-bold text-slate-900 text-sm">
              {t('home.honestData', 'Truthful Fallback States')}
            </h3>
            <p className="text-xs text-slate-600 leading-relaxed">
      ...
      <div className="bg-slate-900 text-white rounded-2xl p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6 shadow-md border border-slate-800">
        ...
        >
          {t('home.exploreProjects', 'Explore Mapped Projects')}
